...
func TestMysqlEmployeeDatabaseSQLRepository_FirstEmployee(t *testing.T) {
			...
			before: func() {
				{
					...
					mockQuery.ExpectQuery(
						regexp.QuoteMeta(
							"SELECT `employees`.`id`,`employees`.`xid`,`employees`.`name`,`employees`.`email`,`employees`.`address`,`employees`.`age`,`employees`.`birthday`,`employees`.`created_at`,`employees`.`updated_at`,`User`.`id` AS `User__id`,`User`.`xid` AS `User__xid`,`User`.`name` AS `User__name` FROM `employees` LEFT JOIN `users` `User` ON `employees`.`user_id` = `User`.`id` WHERE (`employees`.`xid` = ? AND `employees`.`email` = ? AND `employees`.`user_id` = ?) AND `employees`.`deleted_at` IS NULL ORDER BY `employees`.`id` LIMIT 1",
						),
					).
						WithArgs(arg1, arg2, arg3).
						WillReturnError(errors.New("something error"))
			...
			before: func() {
				{
					...
					mockQuery.ExpectQuery(
						regexp.QuoteMeta(
							"SELECT `employees`.`id`,`employees`.`xid`,`employees`.`name`,`employees`.`email`,`employees`.`address`,`employees`.`age`,`employees`.`birthday`,`employees`.`created_at`,`employees`.`updated_at`,`User`.`id` AS `User__id`,`User`.`xid` AS `User__xid`,`User`.`name` AS `User__name` FROM `employees` INNER JOIN `users` `User` ON `employees`.`user_id` = `User`.`id` WHERE (`employees`.`xid` = ? AND `employees`.`email` = ? AND `employees`.`user_id` = ?) AND `employees`.`deleted_at` IS NULL ORDER BY `employees`.`id` LIMIT 1",
						),
					).
						WithArgs(arg1, arg2, arg3).
						WillReturnError(errors.New("something error"))
			...
			before: func() {
				{
					...
					mockQuery.ExpectQuery(
						regexp.QuoteMeta(
							"SELECT DISTINCT `employees`.`id`,`employees`.`xid`,`employees`.`name`,`employees`.`email`,`employees`.`address`,`employees`.`age`,`employees`.`birthday`,`employees`.`created_at`,`employees`.`updated_at`,`User`.`id` AS `User__id`,`User`.`xid` AS `User__xid`,`User`.`name` AS `User__name` FROM `employees` LEFT JOIN `users` `User` ON `employees`.`user_id` = `User`.`id` AND `User`.`name` LIKE ? WHERE (`employees`.`xid` = ? AND `employees`.`email` = ? AND `employees`.`user_id` = ?) AND `employees`.`deleted_at` IS NULL ORDER BY `employees`.`id` LIMIT 1",
						),
					).
	...
	for _, tC := range testCases {
		t.Run(tC.desc, func(t *testing.T) {
			...
			value, err := employeeDatabaseSQLRepository.FirstEmployee(tC.input.query)
